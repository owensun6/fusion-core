...
> **融合来源**: Superpowers finishing-a-development-branch（合并选项流程）+ fusion-workflow Stage 7 规约 + Gate 3 验收条件
...
1. **我们的目的是什么？**
   → 在 Gate 3 通过后，以最安全、最可追溯的方式将 feature 分支的代码合入主线，确保无遗留脏代码、无未解决问题。
2. **这些步骤已经不可原子级再分了吗？**
...
Worktree: [已清理 / 保留待用]
─────────────────────────
可选: 调用 /fusion-extract-genes 提取本次战役经验
```
