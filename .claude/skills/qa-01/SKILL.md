...
> **Stage 6 第一道漏斗** | 融合来源: ECC qa-01 + Fusion-Core testing.md 80% 基线 → Fusion
...
2. **这些步骤已经不可原子级再分了吗？**
   → 运行测试 → 检查覆盖率 → 扫描空指针 → 检查循环终止 → 对照 TASK_SPEC 验收 → 检查错误处理，每步独立。
...
3. 串行管道约束：本道 PASS 后方可通知 qa-02；FAIL 时后续道次不得启动
