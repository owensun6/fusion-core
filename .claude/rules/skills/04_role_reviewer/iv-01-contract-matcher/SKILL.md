---
name: iv-01-contract-matcher
description: Reviewer 军团的集成接生队 1 号。专盯 API Mock 是否能和客户端齿轮咬合。
---

# `iv-01-contract-matcher` (Level 3 Router)

## 执行序列 (Action Sequence)

1. **工具调用**: `view_file` -> `actions/01_Integration.md`
2. **强制规则**: 必须发端到端请求进行对接。
3. **输出信号**: 校验通过则进入造影巡查阶段。
