...
## 0. Gate 提交前置条件：对抗性 FP 审计（所有 Gate 强制）

任何角色在提交 Gate 审批前，**必须由对应的 Consultant 角色执行审计**，并将审计结论附在提交信息中。

| Gate | 审计角色 |
|------|---------|
| Gate 0 | PM Consultant |
| Gate 0.5 | UX Consultant |
| Gate 1 | Architecture Consultant |
| Gate 2 | Lead 自审（对抗性提问，见下方模板） |
| Gate 3 | qa-01 串行管道已覆盖，无需额外 |

**Gate 2 对抗性提问模板**（Lead 必须逐条回答，不得跳过）：

```
Q1. 这批任务覆盖的目的是什么？用一句话说清楚，禁止列举任务本身。
Q2. 走一遍完整的端到端动作链（[主体] 启动 → … → 得到结果）。
    链上哪些环节当前没有对应 Task？为什么可以没有？
Q3. 有哪个 Task 是因为"这类项目一般都有"加进来的？
    删掉它，目的能达成吗？
Q4. 有哪两个 Task 合并成一个后，仍能独立交付？如果有，为什么要拆？
```

缺少审计结论 → Gate 拒绝受理，不讨论内容。

> **为什么不是文字模板，也不依赖外部技能？**
> 文字模板（FP-1/FP-2）导致被审计方自我报告，天然失效。
> 外部技能（fusion-fp-check）在 init 后的新项目中不保证可用。
> Consultant 角色是 fusion-core 内置角色，对抗性提问是固定问题而非自由填空——两个问题都解决了。
